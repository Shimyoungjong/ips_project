# 취약 서버 (진짜 서버, 포트 5000)
# honeypot_flask.py가 이 파일의 "완전 클론"이라는 주석을 갖고 있어 원본 기준으로 복원함.
# 우분투 VM에 ~/vulnerable_server/app.py 로 배포해서 실행.
...
DB_PATH      = '/tmp/vulnerable_users.db'
LOG_FILE     = '/tmp/vulnerable_log.txt'
...
def send_to_detect_http(ip, payload):
    """/search, /board 등에 들어온 값 하나하나를 SQLi/XSS ML 분류기로 보낸다."""
    if not payload:
        return
    try:
...
def log_request(extra=''):
    ...
    if extra:
        line += f"\n  └ PAYLOAD: {extra}"
    print(f"\033[92m[취약서버]\033[0m {line}")
    try:
...
def search():
    ...
    query = ''
    searched = False
    # GET ?q= 로 들어오는 요청도 검색으로 처리 (DDoS 시연 스크립트 호환용)
...
if __name__ == '__main__':
    init_db()
    print(f"[취약서버] 포트 5000 시작")
    print(f"[취약서버] 로그: {LOG_FILE}")
